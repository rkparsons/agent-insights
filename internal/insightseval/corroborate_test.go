...
func TestCorroborateTwoSided(t *testing.T) {
	...
	if got := Corroborate(corroItem("client-project", 4, "a1", "a2"), "client-project", anchors, nil); got != CorroborationOK {
		t.Fatalf("exact 50%% must corroborate: %s", got)
	}
	if got := Corroborate(corroItem("client-project", 4, "a1"), "client-project", anchors, nil); got != CorroborationMismatch {
		t.Fatalf("25%% must mismatch: %s", got)
	}
	// 14 sessions ≤ 3×4+2 passes; 15 breaches the cap (mega-theme)
	if got := Corroborate(corroItem("client-project", 14, "a1", "a2", "a3", "a4"), "client-project", anchors, nil); got != CorroborationOK {
		t.Fatalf("cap boundary must pass: %s", got)
	}
	if got := Corroborate(corroItem("client-project", 15, "a1", "a2", "a3", "a4"), "client-project", anchors, nil); got != CorroborationSizeCap {
		t.Fatalf("mega-theme must fail the cap: %s", got)
	}
	if got := Corroborate(corroItem("tmux-ctrl", 4, "a1", "a2"), "client-project", anchors, nil); got != CorroborationCrossBucket {
		t.Fatalf("non-expected bucket skips corroboration: %s", got)
	}
	if got := Corroborate(corroItem("client-project", 3), "client-project", nil, nil); got != CorroborationNoAnchors {
...
func TestCorroborateCapCountsAgainstSourceTheme(t *testing.T) {
	...
	if got := Corroborate(corroItem("client-project", 15, "a1", "a2", "a3", "a4"), "client-project", kept, capAnchors); got != CorroborationOK {
		t.Fatalf("cap must count against the source theme, not kept anchors: %s", got)
	}
	// the source-theme cap still bites: 21 > 3×6+2
	if got := Corroborate(corroItem("client-project", 21, "a1", "a2", "a3", "a4"), "client-project", kept, capAnchors); got != CorroborationSizeCap {
	...
	if got := Corroborate(corroItem("client-project", 4, "a1", "a2"), "client-project", kept, capAnchors); got != CorroborationOK {
