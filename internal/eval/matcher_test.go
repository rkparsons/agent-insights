...
func samplePayload() MatchPayload {
	return MatchPayload{
		...
		Items: []MatchItem{{ID: "finding/1", Repos: []string{"alpha", "beta"}, Surface: surfaceFinding, Text: "Confident conclusions before verifying"}},
	}
}

func TestClaudeMatcherParsesEnvelope(t *testing.T) {
	fr := &fakeRunner{out: `{"is_error":false,"result":"","structured_output":` +
		`{"matches":[{"item_id":"finding/1","granularity":"partial","nuance_results":[false],"forbidden_forms_matched":[]}]}}`}
	m := claudeMatcher{run: fr.run}
	res, err := m.Match(context.Background(), samplePayload())
	if err != nil {
		t.Fatal(err)
	}
	if len(res.Matches) != 1 || res.Matches[0].Granularity != "partial" || res.Matches[0].ItemID != "finding/1" {
		t.Fatalf("res: %+v", res)
	}
...
func TestClaudeMatcherRejectsInconsistentOutput(t *testing.T) {
	...
	cases := []struct{ name, out string }{
		{"unknown item", `{"structured_output":{"matches":[{"item_id":"finding/9","granularity":"full","nuance_results":[true],"forbidden_forms_matched":[]}]}}`},
		{"nuance count", `{"structured_output":{"matches":[{"item_id":"finding/1","granularity":"full","nuance_results":[],"forbidden_forms_matched":[]}]}}`},
		{"forbidden index", `{"structured_output":{"matches":[{"item_id":"finding/1","granularity":"full","nuance_results":[true],"forbidden_forms_matched":[3]}]}}`},
		{"bad granularity", `{"structured_output":{"matches":[{"item_id":"finding/1","granularity":"absent","nuance_results":[true],"forbidden_forms_matched":[]}]}}`},
	}
	for _, tc := range cases {
		m := claudeMatcher{run: (&fakeRunner{out: tc.out}).run}
		if _, err := m.Match(context.Background(), samplePayload()); err == nil {
