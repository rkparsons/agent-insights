...
func Extract(events []claude.TranscriptEvent, canary claude.Canary, sessionID string, repo RepoResolver) SessionExtraction {
	return extractWithBudget(events, canary, sessionID, repo, defaultBudget)
}

func extractWithBudget(events []claude.TranscriptEvent, canary claude.Canary, sessionID string, repo RepoResolver, budget int) SessionExtraction {
