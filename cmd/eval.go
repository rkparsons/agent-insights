...
func parseOutcomeArgs(args []string) (eval.OutcomeOptions, error) {
	opts := eval.OutcomeOptions{DataDir: defaultDataDir(), CacheDir: defaultCacheDir()}
	for i := 0; i < len(args); i++ {
...
func sortedBucketKeys(buckets map[string]eval.BucketPopulations) []string {
...
func parseScoreArgs(args []string) (eval.ScoreOptions, error) {
	opts := eval.ScoreOptions{DataDir: defaultDataDir(), CacheDir: defaultCacheDir()}
	for i := 0; i < len(args); i++ {
		next := func() (string, error) {
			i++
			if i >= len(args) {
...
func runEvalScore(args []string) {
	...
	v, arts, err := eval.ScoreRun(context.Background(), opts)
...
func runEvalScoreTargets(opts eval.ScoreOptions) {
	results, warnings, err := eval.ScoreTargets(context.Background(), opts)
...
func printVerdict(v eval.Verdict, arts eval.ScoreArtifacts) {
	status := "PASS"
	if v.HardFail {
		status = "HARD FAIL"
	}
