...
- **Worktree targeting**: auto-named worktrees (happy-stork, proud-mesa…) are review-only; fixes go in the ticket worktree (`TICKET-NNNNN`) — you had to redirect twice; default to asking which worktree when a fix emerges mid-review.
- **Generated-manifest rebases**: `packages/prompts/manifest.json` is generated (the package build runs `build-manifest`); on rebase conflict regenerate via `pnpm --filter @org/prompts build` and restage — never hand-merge. Hit in 2 sessions.
...
OOM-killed vitest, one lost/stalled backend-api session.
